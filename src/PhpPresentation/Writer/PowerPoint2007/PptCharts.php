...
class PptCharts extends AbstractDecoratorWriter
{
    ...
    protected function writeSpreadsheet(PhpPresentation $presentation, Chart $chart, string $tempName): string
    {
        ...
        foreach ($chart->getPlotArea()->getTypes() as $type) {
            foreach ($type->getSeries() as $series) {
                // Title
    ...
    protected function writeTypeArea(XMLWriter $objWriter, Area $subject, bool $includeSheet = false,int $seriesIndex=0): void
    {
        ...
        foreach ($subject->getSeries() as $series) {
            ...
            $this->writeMultipleValuesOrReference($objWriter, $includeSheet, $axisYData, $coords, $series->getFormatCode());
            $objWriter->endElement();
    ...
    protected function writeAxis(XMLWriter $objWriter, Chart\Axis $oAxis, string $typeAxis, Chart\Type\AbstractType $typeChart): void
    {
        ...
        $objWriter->writeAttribute('sourceLinked', $oAxis->getFormatCode() != '' ? '0' : '1');
        $objWriter->endElement();
